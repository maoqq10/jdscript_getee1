...
!(async () => {
  if (!cookiesArr[0]) {
    $.msg('【京东账号一】电脑数码失败', '【提示】请先获取京东账号一cookie\n直接使用NobyDa的京东签到获取', 'https://bean.m.jd.com/bean/signIndex.action', {"open-url": "https://bean.m.jd.com/bean/signIndex.action"});
  }
...
async function help(inviteId){
    await isvObfuscator()
    if($.isvObfuscatorToken && $.isvObfuscatorToken.token){
        await auth($.isvObfuscatorToken.token)
        if($.authToken){
            try {
        ...
        $.log('', `❌ ${$.name}, isvObfuscator失败! 原因: ${$.isvObfuscatorToken.message}!`, '')
        return
    }
}
    ...
    $.post(option, (err, resp, data) => {
      try {
        if (err) {
          ...
          console.log('isvObfuscator', data)
          if (safeGet(data)) {
            $.isvObfuscatorToken = JSON.parse(data);
        ...
        try {
          if (err) {
              if(err.status_code == 422){
                ...
                console.log(`invite 错误：${JSON.stringify(err)}`, data)
                console.log(`${$.name} API请求失败，请检查网路重试`)
              }
              reject()
          } else {
