...
async function joyReward() {
  ...
  if ($.getExchangeRewardsRes && $.getExchangeRewardsRes.success) {
    ...
    let giftSaleInfosKey = 'beanConfigs0';
    let time = new Date($.getExchangeRewardsRes['currentTime']).getHours();
    if (time >= 23 && time < 7) {
      giftSaleInfosKey = 'beanConfigs0';
    }
    if (time >= 7 && time < 15) {
      giftSaleInfosKey = 'beanConfigs8';
    }
    if (time >= 15 && time < 23) {
      giftSaleInfosKey = 'beanConfigs16';
    }
    const giftSaleInfos = data[giftSaleInfosKey];
...
function getExchangeRewards() {
  return new Promise((resolve) => {
    const option = {
      url: `${JD_API_HOST}/common/gift/getBeanConfigs?reqSource=h5&lks=4966e661c750af0577ea7a5e3fa5337f&lkt=1622978332443&invokeKey=Oex5GmEuqGep1WLC`,
      headers: {
        "Host": "jdjoy.jd.com",
...
function exchange(saleInfoId, orderSource) {
  return new Promise((resolve) => {
    const option = {
      url: `${JD_API_HOST}/common/gift/new/exchange?reqSource=h5&lks=4fc78d29fc827ed38b8a73364c62c8d6&lkt=1622978353698&invokeKey=Oex5GmEuqGep1WLC`,
      body: `${JSON.stringify({'buyParam': { saleInfoId, orderSource }, 'deviceInfo':{}})}`,
      headers: {
        "Host": "jdjoy.jd.com",
        "Accept": "*/*",
        "Accept-Encoding": "gzip, deflate, br",
...
function getJDServerTime() {
  return new Promise(resolve => {
    ...
    $.get({url: `${JD_API_HOST}/system/current/timestamp`,headers:{
