...
let tuanActiveId = `9rjG4ynoRkoB7xaWk_i6bw==`;
const jxOpenUrl = `openjd://virtual?params=%7B%20%22category%22:%20%22jump%22,%20%22des%22:%20%22m%22,%20%22url%22:%20%22https://wqsd.jd.com/pingou/dream_factory/index.html%22%20%7D`;
let cookiesArr = [], cookie = '', message = '';
const inviteCodes = [];
const jdCookieNode = $.isNode() ? require('./jdCookie.js') : '';
var shareCodeType = "dreamFactory";
if ($.isNode()) {
    ...
    $.get(options, (err, resp, data) => {
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            data = JSON.parse(data);
            // console.log('QueryActiveConfig', data)
            if (data['ret'] === 0) {
            ...
            if (data['ret'] === 0) {
              ...
              for (let item of tuanInfo) {
                if (item.tuanNum === item.realTuanNum) {
                  //console.log(`参加团主【${item.tuanLeader}】已成功`, item)
...
function requireConfig() {
  return new Promise(async resolve => {
    const readShareCodeRes = await shareCodeApi.readShareCode('dreamFactoryTuan');

    if (readShareCodeRes && readShareCodeRes.code === 1) {
      $.tuanIdS = readShareCodeRes.data || [];
    }
