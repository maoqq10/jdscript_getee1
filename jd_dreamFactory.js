    ...
    for (let code of $.newShareCodes) {
      if (code) {
        ...
        if (assistFriendRes && assistFriendRes['ret'] === 0) {
          ...
          if(assistFriendRes.msg.indexOf('已经满员')>0){
          ...
          if (safeGet(data)) {
            ...
            if (data['ret'] === 0) {
              ...
              if (data.factoryList && data.productionList) {
                ...
                addShareCode($.UserName, data.user.encryptPin)
...
function addShareCode($pt_pin, $code) {
  //console.log(`addShareCode`,$pt_pin, $code)
  return new Promise(async resolve => {
    $.get({url: `https://admin.0xaa.cn/api/share_code/add?type=dreamFactory&code=${$code}&pt_pin=${$pt_pin}`, 'timeout': 20000}, (err, resp, data) => {
        //console.log('addShareCode result:', err, data);
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `https://admin.0xaa.cn/api/share_code/query/type/dreamFactory/limit/0`, timeout: 10000,}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
